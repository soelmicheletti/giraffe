...
def generate_data():
    motif = pd.read_csv("data/breast/raw/motif.txt", sep="\t", header=None)
    ...
    for i in range(motif_filter.shape[0]):
        ...
        if g in motif_matrix.index and tf in motif_matrix.columns:
            motif_matrix[tf][g] = 1
    motif_matrix.to_csv("data/breast/motif.csv")
    ...
    chip_raw = pd.read_csv("data/breast/raw/TF-Target-information.txt", on_bad_lines='skip', sep='\t')
    chip_raw = chip_raw[chip_raw['tissue'] == 'breast']
