...
if not preprocessing:
  reg = giraffe.Giraffe(expression_male_lung.to_numpy(), motif_male.to_numpy(), ppi.to_numpy(), iterations = 1).get_regulation()
...
```{python}
if not preprocessing:
  reg = giraffe.Giraffe(expression_male_luad.to_numpy(), motif_male.to_numpy(), ppi.to_numpy(), iterations = 20).get_regulation()
...
```{python}
if not preprocessing:
  reg = giraffe.Giraffe(expression_female_lung.to_numpy(), motif_female.to_numpy(), ppi.to_numpy(), iterations = 20).get_regulation()
...
```{python}
if not preprocessing:
  reg = giraffe.Giraffe(expression_female_luad.to_numpy(), motif_female.to_numpy(), ppi.to_numpy(), iterations = 20).get_regulation()
...
```{r}
tftar_male_lung <- rowSums(py$giraffe_male_lung)
genetar_male_lung <- colSums(py$giraffe_male_lung)

tftar_male_luad <- rowSums(py$giraffe_male_luad)
genetar_male_luad <- colSums(py$giraffe_male_luad)

tftar_female_lung <- rowSums(py$giraffe_female_lung)
genetar_female_lung <- colSums(py$giraffe_female_lung)

tftar_female_luad <- rowSums(py$giraffe_female_luad)
genetar_female_luad <- colSums(py$giraffe_female_luad)
```
...
fgseaRes <- fgsea(pathways, genetar_male_lung - genetar_male_luad, minSize=15, maxSize=500, nperm=10000)
