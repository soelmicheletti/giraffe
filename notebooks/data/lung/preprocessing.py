...
def generate_data():
    motif = pd.read_csv("data/lung/raw/motif.txt", sep="\t", header=None)
    ...
    for i in range(motif_filter.shape[0]):
        ...
        if g in motif_matrix.index and tf in motif_matrix.columns:
            motif_matrix[tf][g] = 1
    motif_matrix.to_csv("data/lung/motif.csv")
    ...
    for i in ppi.columns:
        ...
        for r in res:
            ...
            ppi[i][r] = 1
            ppi[r][i] = 1
    ppi.to_csv("data/lung/ppi_matrix.csv")
    motif = pd.read_csv("data/lung/motif.csv", index_col = 0)
    chip = pd.DataFrame(0, index=motif.index, columns=motif.columns)
    chip_raw = pd.read_csv("data/lung/raw/TF-Target-information.txt", on_bad_lines='skip', sep='\t')
    chip_raw = chip_raw[chip_raw['tissue'] == 'lung']
    ...
    chip.to_csv("data/lung/chip.csv")
