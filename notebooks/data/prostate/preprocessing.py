...
def generate_data():
    motif = pd.read_csv("data/prostate/raw/motif.txt", sep="\t", header=None)
    motif_filter = motif[motif[2] == 1]
    expression = pd.read_csv("data/prostate/raw/expression.csv", index_col = 0)
    ...
    for i in range(motif_filter.shape[0]):
        ...
        if g in motif_matrix.index and tf in motif_matrix.columns:
            motif_matrix[tf][g] = 1
    motif_matrix.to_csv("data/prostate/motif.csv")
    ...
    chip_raw = chip_raw[chip_raw['tissue'] == 'prostate']
    translation = pd.read_csv("data/prostate/raw/gen_v26_mapping.csv")
    ...
    chip.to_csv("data/prostate/chip.csv")
